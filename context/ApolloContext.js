...
const httpLink = new HttpLink({
	uri:  process.env.NEXT_LENS_PUBLIC_API_URL,
	fetch,
});

// const httpLink = new HttpLink({
// 	uri: `${process.env.NEXT_STEPZEN_API_URL}`,
// 	headers: {
// 					Authorization: `${process.env.NEXT_STEPZEN_API_KEY}`,
// 				},
//   fetch,
// });
...
function ApolloContextProvider({ children }) {
  ...
  const apolloClientZen = new ApolloClient({
		// link: authLink.concat(httpLink),
    // link:httpLink,
    uri: process.env.NEXT_STEPZEN_API_URL,
		cache: new InMemoryCache(),
  ...
  const tx = await lensHub.commentWithSig({
	...
	async function followWithSig(typedData) {
		...
		console.log("create post: signature", signature);
    console.log("proccessing signing");
		const { v, r, s } = splitSignature(signature);
		const signer = await wallet.getSigner();
		const lensHub = new ethers.Contract(
			"0x60Ae865ee4C725cd04353b5AAb364553f56ceF82 ",
      // "0x8C1f82e8AAD9399f52DcF224b77f33d5c1719241",
			LENS_HUB_ABI,
			signer
		);
